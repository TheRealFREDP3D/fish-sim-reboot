...
class BrainVisualizer:
    ...
    def update(self, dt, selected_fish):
        ...
        # Update ripples
        self._ripples = [(a + dt, m, pos, col)
                         for a, m, pos, col in self._ripples if a < m]
            ...
            if len(inp) == 15:
                for i, node_pos in enumerate(self._pos_in):
                    ...
                    if abs(act) > 0.25 and random.random() < abs(act) * dt * 4:
                        self._ripples.append((0.0, 0.7, node_pos, activation_color(act)))
    ...
    def _draw_header(self, surf, fish, accent, y):
        ...
        stage   = ("Elder"    if fish.age >= elder_t else
                   "Adult"    if fish.age >= adult_t else
                   "Juvenile" if fish.age >= juv_t   else "Larva")
    ...
    def _draw_status_bars(self, surf, fish, accent, y):
        ...
        for label, ratio, color in bars:
            ...
            pygame.draw.rect(surf, tuple(c // 3 for c in color),
                             (bx, y, fw, BAR_H), border_radius=RAD)
            pygame.draw.rect(surf, color, (bx, y, fw, BAR_H), border_radius=RAD)
            # Bright leading edge
            if fw > RAD * 2:
                ex = bx + fw - 3
                pygame.draw.rect(surf, tuple(min(255, c + 80) for c in color),
                                 (ex, y + 1, 3, BAR_H - 2), border_radius=2)
    ...
    def _draw_network(self, surf, fish, accent, accent_dim, y):
            ...
            for i, sp in enumerate(src_pos):
                ...
                for dp in dst_pos:
                    ...
                    if strength < THRESH:
                        ...
                        alpha_col = tuple(max(0, min(255, int(c * (0.4 + 0.6 * strength)))) for c in col)
                        if len(pts) > 1:
                            pygame.draw.lines(surf, alpha_col, False, pts, max(1, thick - 1))
                        # Bright core
        ...
        for p in self._particles:
            ...
            if net_top <= py <= net_top + NET_H + 10:
                ...
                # Bright core
                pygame.draw.circle(gs, (255, 255, 255, 200), (self._GLOW, self._GLOW), max(1, r - 1))
        ...
        def draw_node(pos, act, r=5, label=None):
            ...
            if strength > 0.15:
                ...
                pygame.draw.circle(gs, halo_color,
                                   (self._GLOW, self._GLOW), halo_r)
            ...
            if label:
                tl = self.f_tiny.render(label, True, lerp_color(TEXT_LABEL, col, strength * 0.6))
        ...
        for g_name, g_slice in INPUT_GROUPS:
            ...
            bx = xs["in"] - 14
            pygame.draw.line(surf, (35, 58, 80),
                             (bx, min(ys_g) - 4), (bx, max(ys_g) + 4), 1)

        for i, pos in enumerate(pos_in):
            draw_node(pos, inp[i] if i < len(inp) else 0.0, r=4,
        ...
        for i, pos in enumerate(pos_out):
            draw_node(pos, out[i] if i < len(out) else 0.0, r=8,
                      label=OUT_LABELS[i])

        # Column headers
        for label, x in [("IN(15)", xs["in"]), ("H1(12)", xs["h1"]),
    ...
    def _draw_outputs(self, surf, fish, accent, y):
        ...
        if fw > 0:
            ...
            pygame.draw.rect(surf, tuple(c // 3 for c in col),
                             (fx, y + 1, fw, gh - 2), border_radius=(gh - 2) // 2)
            pygame.draw.rect(surf, col,
                             (fx, y + 1, fw, gh - 2), border_radius=(gh - 2) // 2)
        nx = mid + int(steer * gw / 2)
        pygame.draw.line(surf, TEXT_PRIMARY, (nx, y - 3), (nx, y + gh + 3), 2)
        surf.blit(self.f_tiny.render(f"{steer:+.2f}", True, TEXT_SECONDARY),
                  (gx + gw + 4, y + 2))
        y += 22
        ...
        pygame.draw.rect(surf, tuple(c // 3 for c in col),
                         (gx, y, fw, gh), border_radius=gh // 2)
        pygame.draw.rect(surf, col, (gx, y, fw, gh), border_radius=gh // 2)
        surf.blit(self.f_tiny.render(f"{int(thrust_n * 100)}%", True, TEXT_SECONDARY),
                  (gx + gw + 4, y + 2))
        y += 22
        ...
        if len(fish.output_history) > 4:
            ...
            for ch_idx, (ch_label, col_line) in enumerate(
                ...
                surf.blit(self.f_tiny.render(ch_label, True, TEXT_LABEL),
                          (PAD, y + SH // 2 - 6))
                sx    = PAD + 42
                sw    = SW - 42
                mid_y = y + SH // 2
                ...
                if len(pts) > 1:
                    ...
                    pygame.draw.lines(surf, tuple(c // 3 for c in col_line), False, shadow, 1)
                    pygame.draw.lines(surf, col_line, False, pts, 1)
                    # Bright dot at latest value
                    pygame.draw.circle(surf, tuple(min(255, c + 80) for c in col_line),
                                       pts[-1], 2)
                y += SH + 6
        return y + 4
    ...
    def _draw_traits(self, surf, fish, accent, y):
        ...
        for label, val in rows:
            ...
            pygame.draw.rect(surf, tuple(c // 3 for c in col),
                             (fx, y + 1, fw, TH - 2), border_radius=(TH - 2) // 2)
            pygame.draw.rect(surf, col,
                             (fx, y + 1, fw, TH - 2), border_radius=(TH - 2) // 2)
            surf.blit(self.f_tiny.render(f"{val:.2f}", True, TEXT_LABEL),
                      (tx + TW + 4, y + 1))
            y += 17
        return y + 4
    ...
    def _draw_stats(self, surf, fish, accent, y):
        ...
        for i, (label, val) in enumerate(stats):
            ...
            pygame.draw.rect(surf, tuple(c // 5 for c in accent),
                             (bx - 1, y - 1, BOX_W + 2, BOX_H + 2), border_radius=7)
