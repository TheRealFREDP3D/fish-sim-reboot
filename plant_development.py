...
from config import (
    ...
    GERMINATION_FAILURE_ENERGY_THRESHOLD, GERMINATION_FAILURE_TIME,
    SEEDLING_DEATH_TIME, DORMANT_ENERGY_MINIMUM, DORMANT_DEATH_TIME,
    SPRING_GERMINATION_BASE_CHANCE, SUMMER_GERMINATION_BASE_CHANCE,
    SEED_ENERGY_THRESHOLD, FLOWERING_BASE_CHANCE, AUTUMN_SEED_BASE_PROBABILITY,
    SUMMER_SEED_BASE_PROBABILITY, SEED_ENERGY_COST,
)
...
class PlantDevelopment:
    ...
    def update(self, dt, growth_nutrients, height_unused,
        ...
        if self.stage not in ("seed", "dormant", "decomposing"):
            ...
            if season_index == 3:
                maintenance *= 1.4   # winter stress (reduced from 1.6 for better survival)
        ...
        if self.stage == "seed":
            ...
            elif self.time_in_stage > GERMINATION_FAILURE_TIME and self.energy < GERMINATION_FAILURE_ENERGY_THRESHOLD:
    ...
    def _update_as_seed(self, dt, season):
        ...
        if self.energy > SEED_ENERGY_THRESHOLD and random.random() < germ_chance * 60 * dt:
    ...
    def _update_dormant(self, dt, season):
        ...
        self.energy = max(DORMANT_ENERGY_MINIMUM, self.energy - 0.003 * dt)  # reduced from 0.004

        # If energy runs out mid-dormancy, the plant dies
        if self.energy <= DORMANT_ENERGY_MINIMUM and self.time_in_stage > DORMANT_DEATH_TIME:
            self._transition("dying")
    ...
    def _try_enter_flowering(self, season_index):
        ...
        if (self.energy >= FLOWERING_ENERGY_THRESHOLD and
                random.random() < FLOWERING_BASE_CHANCE * pref):  # increased from 0.012
    ...
    def can_produce_seed(self, seed_dispersal_modifier, season_index):
        ...
        energy_cost = SEED_ENERGY_COST  # reduced from 2.5 to make seeding more affordable
