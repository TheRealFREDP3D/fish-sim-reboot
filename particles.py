...
import math
from config import (
    SCREEN_WIDTH, SCREEN_HEIGHT, WATER_LINE_Y,
    PARTICLE_MIN_SIZE, PARTICLE_MAX_SIZE,
    PARTICLE_MAX_SPEED,
    SEDIMENT_COLOR, PLANKTON_COLOR, PARTICLE_ALPHA,
    SEDIMENT_COUNT, PLANKTON_COUNT
)
...
class Particle:
    ...
    def reset(self):
        ...
        self.y = random.uniform(WATER_LINE_Y + 10, SCREEN_HEIGHT - 20)
        self.size = random.randint(PARTICLE_MIN_SIZE, PARTICLE_MAX_SIZE)
        speed_factor = 0.6 if self.is_plankton else 1.0
        self.speed_x = random.uniform(-PARTICLE_MAX_SPEED, PARTICLE_MAX_SPEED) * speed_factor
        self.speed_y = random.uniform(0.05, 0.2) if self.is_plankton else random.uniform(0.05, 0.3)
    ...
    def update(self, time):
        ...
        if self.y < WATER_LINE_Y + 5 or self.y > SCREEN_HEIGHT - 5:
            self.reset()

class ParticleSystem:
    def __init__(self):
        self.particles = []
        for _ in range(SEDIMENT_COUNT):
    ...
    def draw(self, screen):
        ...
        for particle in self.particles:
            ...
            pygame.draw.circle(self.particle_surface, color_with_alpha, 
