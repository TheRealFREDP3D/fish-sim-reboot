...
class NeuralNet:
    ...
    def __init__(self, input_size, hidden_size=8, output_size=9):
        ...
        self.w3 = [[random.uniform(-scale3, scale3) for _ in range(self.hidden2_size)] for _ in range(self.output_size)]
    ...
    def forward(self, inputs):
        ...
        for i in range(self.output_size):
            s = sum(h2[j] * self.w3[i][j] for j in range(self.hidden2_size)) + self.b3[i]
            raw.append(s)

        outputs = [
            self.tanh(raw[0]), self.tanh(raw[1]),   # Movement
            self.sigmoid(raw[2]), self.sigmoid(raw[3]) # Drive
        ] + self.softmax(raw[4:9]) # States
    ...
    def blend(p1, p2):
        ...
        def cross_m(m1, m2): return [[m1[i][j] if random.random() < 0.5 else m2[i][j] for j in range(len(m1[0]))] for i in range(len(m1))]
        def cross_v(v1, v2): return [v1[i] if random.random() < 0.5 else v2[i] for i in range(len(v1))]
    ...
    def mutate(self, rate=0.1, strength=0.2):
        ...
        def mut_l(lst, s):
            for i in range(len(lst)):
                if isinstance(lst[i], list): mut_l(lst[i], s)
                elif random.random() < rate: lst[i] = max(-2.0, min(2.0, lst[i] + random.gauss(0, s)))
        ...
        mut_l(child.w1, strength * 1.2); mut_l(child.b1, strength * 1.2)
        mut_l(child.w2, strength); mut_l(child.b2, strength)
        mut_l(child.w3, strength * 0.5); mut_l(child.b3, strength * 0.5)
        return child