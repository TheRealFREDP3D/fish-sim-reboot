...
class PoopParticle:
    ...
    def draw(self, screen):
        ...
        for i in range(5):
            ...
            pts.append((self.x + math.cos(angle) * dist, self.y + math.sin(angle) * dist))
        pygame.draw.polygon(screen, self.color, pts)

class FishEgg:
    def __init__(self, x, y, traits, parent1=None, parent2=None, is_cleaner=False, is_predator=False):
        self.x, self.y = x, y
        self.traits = traits
    ...
    def draw(self, screen):
        ...
        screen.blit(surf, (int(self.x - 10), int(self.y - 10)))
...
class NeuralFish:
    ...
    def __init__(self, world, traits=None, brain=None, is_cleaner=False):
        ...
        start_y = random.uniform(WATER_LINE_Y + 100, SCREEN_HEIGHT - 200)
    ...
    def get_radar_inputs(self, all_fish, targets, plant_manager):
        ...
        for plant in plant_manager.plants:
            dist_to_base = math.hypot(self.physics.pos[0] - plant.x, self.physics.pos[1] - plant.base_y)
        ...
        def fill_radar(objects, offset, is_threat_radar=False):
            for obj in objects:
                if is_threat_radar and getattr(obj, 'is_hidden', False):
                    continue
                ox = getattr(obj, 'x', obj.physics.pos[0] if hasattr(obj, 'physics') else 0)
                oy = getattr(obj, 'y', obj.physics.pos[1] if hasattr(obj, 'physics') else 0)
                dx, dy = ox - self.physics.pos[0], oy - self.physics.pos[1]
        ...
        mates = [f for f in all_fish if f.is_cleaner == self.is_cleaner and f.sex != self.sex and f.is_mature]
        fill_radar(mates, 6)
    ...
    def update(self, dt, all_fish, targets, particle_system, plant_manager):
        ...
        elif self.is_mature and self.energy > FISH_MATING_THRESHOLD and self.mating_cooldown <= 0:
        ...
        stats = [
            ...
            self.stamina / 100.0,
            (self.physics.pos[1] - WATER_LINE_Y) / (SCREEN_HEIGHT - WATER_LINE_Y),
            math.hypot(*self.physics.vel) / FISH_MAX_SPEED,
            self.security_level
        ]
        ...
        if self.state == FishState.FLEEING:
            if self.closest_plant:
                self.physics.apply_force(self.physics.seek(self.closest_plant.x, self.closest_plant.base_y, weight=2.0))
            max_speed_mod = 1.3
        elif self.state == FishState.NESTING:
            if self.closest_plant:
                self.physics.apply_force(self.physics.seek(self.closest_plant.x, self.closest_plant.base_y, weight=1.5))
                if math.hypot(self.physics.pos[0] - self.closest_plant.x, self.physics.pos[1] - self.closest_plant.base_y) < 40:
                    self.is_pregnant = False
                    partner = self.pregnancy_partner
                    self.pregnancy_partner = None
                    return ("egg", self.physics.pos[0], self.physics.pos[1], self.pregnancy_traits, self, partner)
        elif self.state == FishState.RESTING:
            max_speed_mod = 0.3
            if self.closest_plant and not self.is_hidden:
                self.physics.apply_force(self.physics.seek(self.closest_plant.x, self.closest_plant.base_y, weight=0.5))

        # Family behavior
        if self.family and self.family.active:
            if self.is_mature and not self.is_pregnant:
                child_positions = [(c.physics.pos[0], c.physics.pos[1]) for c in self.family.children if not c.is_mature]
                if child_positions:
                    avg_x = sum(p[0] for p in child_positions) / len(child_positions)
                    avg_y = sum(p[1] for p in child_positions) / len(child_positions)
                    self.physics.apply_force(self.physics.seek(avg_x, avg_y, weight=0.8))
        ...
        self.physics.apply_force((((math.cos(desired_angle) * FISH_MAX_SPEED * max_speed_mod) - self.physics.vel[0]) * 0.1,
                                  ((math.sin(desired_angle) * FISH_MAX_SPEED * max_speed_mod) - self.physics.vel[1]) * 0.1))
        force_mag = thrust_input * self.physics.max_force * (1.8 if self.state == FishState.FLEEING else 1.0)
        self.physics.apply_force((math.cos(self.physics.heading) * force_mag, math.sin(self.physics.heading) * force_mag))

        self.physics.bounce_bounds(0, WATER_LINE_Y + 20, SCREEN_WIDTH, self.world.get_terrain_height(self.physics.pos[0]))
        self.physics.update(dt, FISH_DRAG)

        # Collision with food
        for t in targets[:]:
            tx = getattr(t, 'x', t.physics.pos[0] if hasattr(t, 'physics') else 0)
            ty = getattr(t, 'y', t.physics.pos[1] if hasattr(t, 'physics') else 0)
            if math.hypot(self.physics.pos[0] - tx, self.physics.pos[1] - ty) < 25:
                self.energy = min(FISH_MAX_ENERGY, self.energy + 12.0)
                if hasattr(t, 'reset'): t.reset()
                elif t in targets: targets.remove(t)
                break

        if random.random() < 0.001 * (self.energy / FISH_MAX_ENERGY):
            return PoopParticle(self.physics.pos[0], self.physics.pos[1])
        return None

    def draw(self, screen, time, selected=False):
        size = 5 if self.age < FISH_LARVA_DURATION else (8 if self.is_mature else 6)
        color = self.get_color()
        angle, pos = self.physics.heading, self.physics.pos
        speed = math.hypot(*self.physics.vel)
        wiggle = math.sin(time * (8 + speed * 0.1) + self.age) * (0.2 + speed * 0.002)
        tail_angle = angle + math.pi + wiggle
        tail_pts = [(pos[0], pos[1]),
                    (pos[0] + math.cos(tail_angle - 0.6) * size * 2.5, pos[1] + math.sin(tail_angle - 0.6) * size * 2.5),
                    (pos[0] + math.cos(tail_angle + 0.6) * size * 2.5, pos[1] + math.sin(tail_angle + 0.6) * size * 2.5)]
        ...
        if self.is_pregnant:
            pygame.draw.circle(body_surf, (255, 200, 200, 150), (size * 2.5, size * 2), size * 1.2)
        rotated_body = pygame.transform.rotate(body_surf, -math.degrees(angle))
        screen.blit(rotated_body, rotated_body.get_rect(center=(int(pos[0]), int(pos[1]))))

        eye_x, eye_y = pos[0] + math.cos(angle + 0.35) * size * 1.3, pos[1] + math.sin(angle + 0.35) * size * 1.3
        pygame.draw.circle(screen, (255, 255, 255), (int(eye_x), int(eye_y)), 2)
        pygame.draw.circle(screen, (0, 0, 0), (int(eye_x), int(eye_y)), 1)

        if selected:
            pygame.draw.circle(screen, (255, 255, 200), (int(pos[0]), int(pos[1])), size + 15, 2)
    ...
    def draw_brain(self, screen, time):
        ...
        y_offset = 20
        # Title
        species = "Predator" if self.is_predator else ("Cleaner" if self.is_cleaner else "Common")
        tag_color = (255, 100, 100) if self.is_predator else ((100, 200, 255) if self.is_cleaner else (255, 180, 50))
        surface.blit(self.font_title.render(f"{species} Fish Brain", True, tag_color), (20, y_offset))
        y_offset += 40
        
        # Identity Block
        life_stage = "Larva" if self.age < FISH_LARVA_DURATION else ("Juvenile" if self.age < FISH_LARVA_DURATION + FISH_JUVENILE_DURATION else "Adult")
        ...
        surface.blit(self.font_normal.render(id_text, True, (255, 255, 255)), (20, y_offset))
        ...
        # State display
        surface.blit(self.font_small.render(f"CURRENT FOCUS: {self.state.name}", True, (200, 255, 200)), (20, y_offset))
        y_offset += 25

        # --- Progress Bars ---
        # 1. Life Progress (remaining)
        current_max = FISH_MAX_AGE * self.traits.physical_traits.get("lifespan_mult", 1.0)
        life_ratio = max(0.0, 1.0 - (self.age / current_max))
        surface.blit(self.font_small.render(f"LIFE ({int(life_ratio*100)}%)", True, (200, 200, 200)), (20, y_offset))
        pygame.draw.rect(surface, (40, 40, 40), (100, y_offset + 2, 120, 12))
        pygame.draw.rect(surface, (150, 200, 255), (100, y_offset + 2, int(120 * life_ratio), 12))
        
        # 2. Energy Bar
        energy_ratio = max(0.0, min(1.0, self.energy / FISH_MAX_ENERGY))
        surface.blit(self.font_small.render("ENERGY", True, (200, 200, 200)), (250, y_offset))
        pygame.draw.rect(surface, (40, 40, 40), (320, y_offset + 2, 120, 12))
        pygame.draw.rect(surface, self._get_gradient_color(energy_ratio), (320, y_offset + 2, int(120 * energy_ratio), 12))
        
        y_offset += 35
        
        # Network Visualization
        surface.blit(self.font_normal.render("NEURAL ACTIVITY MAP", True, (255, 255, 255)), (20, y_offset))
        y_offset += 40
        
        # Draw connections and nodes (restored logic)
        input_x, hidden_x, output_x = 60, 220, 380
        input_spacing, node_radius = 28, 10
        input_labels = ["Food L", "Food C", "Food R", "Threat L", "Threat C", "Threat R", 
                       "Mate L", "Mate C", "Mate R", "Energy", "Stamina", "Depth", "Speed", "Safety"]
        ...
        for i, inp_val in enumerate(self.last_inputs):
            ...
            for h, hid_val in enumerate(self.last_hidden):
                ...
                if abs(inp_val) > 0.1:
                    ...
                    pygame.draw.line(surface, (*a_color, 40), (input_x, iy), (hidden_x, hy), 1)
        ...
        for h, hid_val in enumerate(self.last_hidden):
            ...
            for o, out_val in enumerate(self.last_outputs):
                ...
                if abs(hid_val) > 0.1:
                    ...
                    pygame.draw.line(surface, (*a_color, 80), (hidden_x, hy), (output_x, oy), 2)
                    
        # Draw Nodes
        for i, (val, lbl) in enumerate(zip(self.last_inputs, input_labels)):
            ny = y_offset + i * input_spacing
            pygame.draw.circle(surface, self._get_activation_color(val), (input_x, ny), node_radius)
            pygame.draw.circle(surface, (255, 255, 255), (input_x, ny), node_radius, 2)
            surface.blit(self.font_small.render(lbl, True, (220, 220, 220)), (input_x + 18, ny - 6))

        for h, val in enumerate(self.last_hidden):
            ny = y_offset + 80 + h * 50
            pygame.draw.circle(surface, self._get_activation_color(val), (hidden_x, ny), node_radius + 2)
    ...
    def get_color(self):
        if self.is_predator: base = [220, 60, 60]
        elif self.is_cleaner: base = [100, 180, 220]
        else: base = [255, 160, 60]
        color = [max(0, min(255, base[i] + self.traits.color_offset[i])) for i in range(3)]
        factor = 1.0 - (self.age / FISH_MAX_AGE) * 0.4
        return tuple(max(0, min(255, int(c * factor))) for c in color)

class FishSystem:
    def __init__(self, particle_system, plant_manager, world):
        self.world, self.particle_system, self.plant_manager = world, particle_system, plant_manager
        ...
        self.cleaner_fish = [CleanerFish(world) for _ in range(CLEANER_FISH_MAX_POPULATION // 2)]
    ...
    def handle_click(self, pos):
        ...
        if not all_f: return
        clicked = min(all_f, key=lambda f: math.hypot(f.physics.pos[0]-pos[0], f.physics.pos[1]-pos[1]))
        if math.hypot(clicked.physics.pos[0]-pos[0], clicked.physics.pos[1]-pos[1]) < 40:
            self.selected_fish = clicked
        else: self.selected_fish = None
    ...
    def update(self, dt):
        ...
        for predator in self.predators:
            if (hasattr(predator, 'try_reproduce') and predator.try_reproduce() and len(self.predators) < PREDATOR_MAX_POPULATION):
                egg = FishEgg(predator.physics.pos[0], predator.physics.pos[1], predator.traits.mutate(), parent1=predator, parent2=predator.mate, is_predator=True)
        ...
        for family in self.families[:]:
            family.update(dt)
            if not family.active: self.families.remove(family)

        # 4. Handle Sediment/Fertilizer
        for p in self.poops[:]:
            if not p.update(dt, self.world): self.poops.remove(p)
        ...
        for f_list, targets, can_mate in sim_groups:
            for f in f_list[:]:
                res = f.update(dt, all_fish, targets, self.particle_system, self.plant_manager)
                ...
                if isinstance(res, PoopParticle): 
                    ...
                    self.eggs.append(FishEgg(res[1], res[2], res[3], res[4], res[5], f.is_cleaner, f.is_predator))
                
                # Handle death
                lifespan = FISH_MAX_AGE * f.traits.physical_traits.get("lifespan_mult", 1.0)
                if f.energy <= 0 or f.age > lifespan:
                    if f == self.selected_fish: self.selected_fish = None
                    f_list.remove(f)
                    continue
        ...
        if len(self.fish) < 6: self.fish.append(NeuralFish(self.world))

    def try_mate(self, f, f_list):
        if f.is_pregnant: return
        for partner in f_list:
            if partner != f and partner.state == FishState.MATING and partner.sex != f.sex:
                if math.hypot(f.physics.pos[0]-partner.physics.pos[0], f.physics.pos[1]-partner.physics.pos[1]) < 45:
                    ...
                    mother.pregnancy_traits, mother.pregnancy_partner = child_traits, father
                    break
    ...
    def draw(self, screen, time):
        for e in self.eggs: e.draw(screen)
        for p in self.poops: p.draw(screen)
        for f in self.fish + self.cleaner_fish + self.predators:
            f.draw(screen, time, f == self.selected_fish)
        if self.selected_fish: self.selected_fish.draw_brain(screen, time)
