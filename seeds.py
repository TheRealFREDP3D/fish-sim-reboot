...
from config import (
    ...
    KELP_DEPTH_MAX, SEAGRASS_DEPTH_MIN, SEAGRASS_DEPTH_MAX, ALGAE_DEPTH_MIN
)
...
class Seed:
    ...
    def __init__(self, plant_type, traits=None):
        ...
        self.y = random.uniform(WATER_LINE_Y + 20, SCREEN_HEIGHT - 100)
    ...
    def update(self, dt, world):
        ...
        if self.y >= terrain_y - 3:
            ...
            if self.plant_type == 'kelp' and depth_ratio <= KELP_DEPTH_MAX:
                return True
            elif self.plant_type == 'seagrass' and SEAGRASS_DEPTH_MIN <= depth_ratio <= SEAGRASS_DEPTH_MAX:
                return True
    ...
    def draw(self, screen):
        ...
        pygame.draw.circle(screen, color, (int(self.x), int(self.y)), int(draw_radius))
        # Add subtle glow
        glow_color = tuple(min(255, c + 70) for c in color)
        pygame.draw.circle(screen, glow_color, (int(self.x), int(self.y)), int(draw_radius + 2), 1)
